...
首次建立不需要 SMTP 或先驗證 Email。從專案根目錄執行
`./.venv/Scripts/python.exe -m equity.membership_admin migrate`，再雙擊
`setup-owner.cmd`。macOS/Linux 使用 `sh setup-owner.sh`。
工具會要求 Email、密碼與再次輸入密碼；密碼輸入不顯示，
不存入命令列、文件或日誌。沒有預設帳密，也沒有公開網頁的擁有者建立入口。

新建擁有者可在伺服器本機瀏覽器，使用服務的 loopback 網址加上 `/members` 直接登入。
首次尚未完成寄信改密碼前，公開網址及代理轉送請求都不能使用此帳號，
每次管理請求也會再次檢查來源；Email 在資料庫仍記為未驗證。
不會建立通用的 `admin` 預設帳密。只有操作人明確指定臨時密碼時，才可使用
`create-owner --allow-temporary-password`，密碼仍由隱藏輸入提示讀取。
...
後臺按「寄送修改密碼連結」，信寄至帳號保存的 Email。打開信中「設定新密碼」，
輸入新密碼與再次確認即可；連結 15 分鐘內有效、只能使用一次，重寄使舊連結失效。
新密碼至少 10 字且須混合字母、數字或符號。完成後驗證 Email、撤銷舊登入，
並解除初始帳號的本機限制。一般會員帳號頁的 Email 驗證碼流程仍保留。

在 `review_src/.env` 設定 `SMTP_HOST`、`SMTP_PORT`、`SMTP_USER`、`SMTP_PASSWORD`、
`FROM_EMAIL`。未設定或寄送失敗會明確回報，不能把模擬寄信當作真實送達。
連結網址優先使用 `EQUITY_AUTH_PUBLIC_URL`（HTTPS origin、不含路徑）；
未填時讀取 `EQUITY_PUBLIC_ENDPOINT_FILE`，預設為相對專案的
`var/services/public-endpoint.json`，必須是 ready 狀態。永不使用請求 Host 組合連結。
Quick Tunnel 重啟會換網址，舊信可能無法開啟；應重新寄送，正式使用宜設定固定網址。
寄信每分鐘一次、每小時最多五次。資料庫只存連結雜湊，頁面 GET 不消耗連結。
